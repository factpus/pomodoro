...
- [ ] Discord Activityの招待可否が `unknown` になる原因を解消し、招待ダイアログまたはリンク共有へ正しく到達することを実機確認
...
- [ ] ホストのheartbeatが失効したとき、生存中の参加者へホスト管理権限を安全かつ原子的に自動移譲する
- [ ] 接続中の全参加者がタイマーの開始・一時停止・リセット・スキップを操作できるようにし、連携設定とホスト移譲の管理権限はホストだけに維持する
- [ ] Discordの招待権限確認APIが利用できない環境を恒久的な警告扱いにせず、招待ダイアログを試行してRPC結果に応じた理由とリンク共有を提示する
- [ ] Discord Activityの2ユーザー自動参加を再現可能な手順で検証し、同一 `instance_id` が必ず同一ルームへ到達することを確認する
...
### Phase 0.6: 共同操作とホスト自動引き継ぎ（計画）

- タイマー操作は共同作業の参加者全員へ開放し、誰の操作も同じRedis状態へ原子的に反映する
- Webhook設定、ホスト移譲、将来追加する管理操作はホスト権限として分離したままにする
- ホストの参加者TTLが失効し、生存中の参加者がいる場合は、最も早く参加した参加者を次のホストとして決定する
- 新しいホストトークンは選出されたクライアントの認証済みheartbeatにだけ返し、旧トークンを同じRedisロック内で無効化する
...
Developer PortalのApplication作成、Activity URL Mapping、OAuth Redirect、環境変数、Entry Pointの設定は完了し、Discordデスクトップ版でActivityの起動とタイマー操作まで確認した。招待権限確認が不確定になる問題、同一Activityへの2ユーザー自動参加、Discord Web版・iOS・Androidの実機確認は未完了である。手順と確認状況は `DISCORD_SETUP.md` に固定する。
